...
func TestDefaultCatalogUsesCanonicalSpiceRepository(t *testing.T) {
	...
	if toolchain.Status != "active" ||
		...
		toolchain.Release.Version != "v0.1.0-preview.6" ||
...
func TestToolchainPreviewSixAuthorityIsExact(t *testing.T) {
	t.Parallel()
	value, err := Default()
	if err != nil {
		t.Fatal(err)
	...
	const (
		...
		publishedToolchainVersion  = "v0.1.0-preview.5"
		authorizedToolchainVersion = "v0.1.0-preview.6"
		tuiToolchainVersion        = "v0.1.0-preview.4"
	)
	...
	if toolchain.Release == nil || !reflect.DeepEqual(*toolchain.Release, wantToolchain) ||
		...
		t.Fatalf("normalized Toolchain preview.6 delta changed more than release.version: got %#v, want %#v", toolchain.Release, wantToolchain)
	}
	...
	if coding.Release == nil || len(coding.Release.Targets) != 6 || len(coding.Release.Binaries) != 2 ||
		...
		t.Fatalf("Coding distribution policy changed while authorizing Toolchain preview.6: %#v", coding.Release)
	}
	...
	if tui.Release == nil || !reflect.DeepEqual(*tui.Release, wantTUI) {
		t.Fatalf("TUI policy moved with Toolchain preview.6 authority: got %#v, want %#v", tui.Release, wantTUI)
	}
}
