...
func TestRenderAndVerifyDeterministicModuleRelease(t *testing.T) {
	...
	wantFiles := []string{
		"checksums.txt",
		"spice-agent_0.1.0-preview.7_release.json",
		"spice-agent_0.1.0-preview.7_sbom.spdx.json",
		"spice-agent_0.1.0-preview.7_source.tar.gz",
	}
	...
	assertArchive(t, filepath.Join(first, wantFiles[3]), "spice-agent_0.1.0-preview.7/agent.go")
...
func TestRenderAndVerifyDependencyFreeModuleRelease(t *testing.T) {
	...
	archive := filepath.Join(output, "spice-agent_0.1.0-preview.7_source.tar.gz")
	entries := archiveEntries(t, archive)
	for _, unexpected := range []string{
		"spice-agent_0.1.0-preview.7/go.sum",
		"spice-agent_0.1.0-preview.7/vendor/modules.txt",
	} {
		if slices.Contains(entries, unexpected) {
			t.Fatalf("dependency-free source archive unexpectedly contains %q", unexpected)
		}
	}
	sbomContent, err := os.ReadFile(filepath.Join(output, "spice-agent_0.1.0-preview.7_sbom.spdx.json"))
	if err != nil {
		t.Fatal(err)
	}
	var sbom spdxDocument
...
func TestCheckPolicyRequiresExactCatalogAuthorization(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "stale Toolchain preview.6", options: PolicyOptions{Profile: toolchainWant.Profile, Repository: toolchainWant.Repository, Module: toolchainWant.Module, Version: "v0.1.0-preview.6"}, want: "does not match catalog"},
		{name: "stale Toolchain preview.7", options: PolicyOptions{Profile: toolchainWant.Profile, Repository: toolchainWant.Repository, Module: toolchainWant.Module, Version: "v0.1.0-preview.7"}, want: "does not match catalog"},
		...
		{name: "stale distribution preview.4", options: PolicyOptions{Profile: distributionWant.Profile, Repository: distributionWant.Repository, Module: distributionWant.Module, Version: "v0.1.0-preview.4"}, want: "does not match catalog"},
		...
		{name: "stale preview.6", options: PolicyOptions{Profile: want.Profile, Repository: want.Repository, Module: want.Module, Version: "v0.1.0-preview.6"}, want: "does not match catalog"},
...
func TestRenderRejectsUntrustedSourceAndPolicy(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "stale preview.6", mutate: func(value *releaseFixture) { value.options.Version = "v0.1.0-preview.6" }, want: "does not match catalog"},
...
func newReleaseFixture(t *testing.T, options fixtureOptions) releaseFixture {
	...
	metadata := `{"schema":1,"profile":"go-module-v1","repository":"spice-agent","module":"` + intentModule + `","version":"v0.1.0-preview.7"}`
