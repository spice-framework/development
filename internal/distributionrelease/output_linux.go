...
func commitOutput(staging string, output string) error {
	...
	_, _, errno := syscall.Syscall6(
		trap, ^uintptr(99), uintptr(unsafe.Pointer(stagingPointer)),
		^uintptr(99), uintptr(unsafe.Pointer(outputPointer)), 1, 0,
	)
...
func linuxRenameat2Trap() (uintptr, error) {
	...
	case "arm64":
		return 276, nil
	default:
		return 0, fmt.Errorf("atomic no-replace distribution commit is unsupported on linux/%s", runtime.GOARCH)
	}
}
