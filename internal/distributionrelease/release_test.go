...
func TestRenderToolchainRepositoryKeyedDistribution(t *testing.T) {
	...
	wantFiles := []string{
		"checksums.txt",
		"toolchain_0.1.0-preview.5_linux_amd64.tar.gz",
		"toolchain_0.1.0-preview.5_release.json",
		"toolchain_0.1.0-preview.5_sbom.spdx.json",
		"toolchain_0.1.0-preview.5_windows_amd64.zip",
	}
	if result.Commit != fixture.commit || !slices.Equal(result.Files, wantFiles) {
	...
	if metadata.Repository != "toolchain" || metadata.Module != "github.com/spice-framework/toolchain" ||
		metadata.Version != "v0.1.0-preview.5" || len(metadata.Targets) != 2 || len(metadata.Payloads) != 2 ||
...
func TestToolchainPreviewFiveDistributionRequiresPublishedFoundation(t *testing.T) {
	t.Parallel()
	value, err := catalog.Default()
	if err != nil {
		t.Fatal(err)
	}
	repository, err := selectRepository(value, "toolchain", "v0.1.0-preview.5")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !slices.Equal(repository.Release.RequiredModules, want) {
		t.Fatalf("Toolchain preview.5 foundation = %#v, want %#v", repository.Release.RequiredModules, want)
	}
}
...
func assertToolchainTarDistribution(t *testing.T, name string, commit string) {
	...
	reader := tar.NewReader(gzipReader)
	want := map[string]bool{
		"toolchain_0.1.0-preview.5_linux_amd64/spice":     false,
		"toolchain_0.1.0-preview.5_linux_amd64/LICENSE":   false,
		"toolchain_0.1.0-preview.5_linux_amd64/README.md": false,
	}
	for {
		...
		if strings.HasSuffix(header.Name, "/spice") {
			...
			if readErr != nil || header.Mode != 0o755 || len(content) < 4 ||
				...
				!bytes.Contains(content, []byte("0.1.0-preview.5")) ||
